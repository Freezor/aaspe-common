...
public static class ExtendLangStringSet
{
    ...
    {
        return langStringSet is {Count: >= 1};
    }

    #endregion
    public static bool IsEmpty(this List<ILangStringNameType>? langStringSet)
    {
        return langStringSet == null || langStringSet.Count == 0;
    }
    ...
    public static void Set(this List<ILangStringTextType> lss, string lang, string text)
    {
        foreach (var ls in lss.Where(ls => string.Equals(ls.Language.Trim(), lang?.Trim(), StringComparison.CurrentCultureIgnoreCase)))
        {
            ls.Text = text;
        }
    ...
    {
        return sourceLangStrings.langString.Count == 0 
            ? langStringSet 
            : sourceLangStrings.langString.Select(sourceLangString => new LangStringTextType(sourceLangString.lang, sourceLangString.str)).Cast<ILangStringTextType>().ToList();
    }
        ...
        Func<string, string, T> createLs) where T : class
    {
        ...
        while (true)
        {
            ...
            cell = cell[(m.Index + m.Length)..];
        }

        return res;
    }

    public static void Add<T>(this IEnumerable<T> list, string language, string text) where T : IAbstractLangString
    {
        if (typeof(T).IsAssignableFrom(typeof(ILangStringTextType)))
        {
            (list as List<ILangStringTextType>)?.Add(new LangStringTextType(language, text));
        }
        else if (typeof(T).IsAssignableFrom(typeof(ILangStringNameType)))
        {
            (list as List<ILangStringNameType>)?.Add(new LangStringNameType(language, text));
        }
        else if (typeof(T).IsAssignableFrom(typeof(ILangStringPreferredNameTypeIec61360)))
        {
            (list as List<ILangStringPreferredNameTypeIec61360>)?.Add(new LangStringPreferredNameTypeIec61360(language, text));
        }
        else if (typeof(T).IsAssignableFrom(typeof(ILangStringShortNameTypeIec61360)))
        {
            (list as List<ILangStringShortNameTypeIec61360>)?.Add(new LangStringShortNameTypeIec61360(language, text));
        }
        else if (typeof(T).IsAssignableFrom(typeof(ILangStringDefinitionTypeIec61360)))
        {
            (list as List<ILangStringDefinitionTypeIec61360>)?.Add(new LangStringDefinitionTypeIec61360(language, text));
        }
    }
}