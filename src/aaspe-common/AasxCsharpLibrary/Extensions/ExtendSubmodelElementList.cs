...
public static class ExtendSubmodelElementList
{
    ...
    {
        if (submodelElementList == null) return;
    ...
    public static T? FindFirstIdShortAs<T>(this SubmodelElementList submodelElementList, string idShort) where T : ISubmodelElement
    {
        ...
        var submodelElements = submodelElementList.Value.Where(sme => sme.IdShort != null && sme.IdShort.Equals(idShort, StringComparison.OrdinalIgnoreCase));
        ...
        this SubmodelElementList elem, ISubmodelElement? source)
    {
        ...
        switch (source)
        {
            case SubmodelElementCollection {Value: not null} srcColl:
                elem.Value = ExtendISubmodelElement.Copy(srcColl.Value);
                break;
            case Operation srcOp:
            {
                void Appov(IReadOnlyCollection<ISubmodelElement> dst, List<IOperationVariable> src)
                {
                    if (src == null) return;
                ...
                elem.Value = new List<ISubmodelElement>();
