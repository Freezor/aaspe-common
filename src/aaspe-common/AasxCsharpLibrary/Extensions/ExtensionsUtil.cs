...
public static class ExtensionsUtil
{
    ...
    public static Reference? ConvertReferenceFromV10(AdminShellV10.Reference sourceReference, ReferenceTypes referenceTypes)
    {
        ...
        if (sourceReference is not {IsEmpty: false}) return outputReference;
    ...
    {
        Reference? outputReference = null;
        if (sourceReference is not {IsEmpty: false})
        {
            return outputReference;
        }
    ...
    {
        var newLangStrList = sourceDescription?.langString.Select(ls => new LangStringTextType(ls.lang, ls.str)).Cast<ILangStringTextType>().ToList();

        return newLangStrList != null ? new List<ILangStringTextType>(newLangStrList) : null;
    }
