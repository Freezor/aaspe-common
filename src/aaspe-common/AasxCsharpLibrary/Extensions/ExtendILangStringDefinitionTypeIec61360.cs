...
public static class ExtendILangStringDefinitionTypeIec61360
{
    ...
    public static string? GetDefaultString(this List<ILangStringDefinitionTypeIec61360> langStringSet, string defaultLang = "en")
    {
        string? res = null;

        // search
        foreach (var langString in langStringSet.Where(langString => langString.Language.Equals(defaultLang, StringComparison.OrdinalIgnoreCase)))
            res = langString.Text;
